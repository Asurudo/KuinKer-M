...
对于随机图来说，$SPFA$ 的时间复杂度为 $O(km)$，其中 $k$ 是一个较小的常数，但是对于单独构造~~坑爹~~数据来说，其时间复杂度很可能会退化为 $O(nm)$，即使加上 $SLF$ 优化亦如此，因此谨慎使用！
